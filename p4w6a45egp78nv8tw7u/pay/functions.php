...
class TransferRequest {
	...
	function addTransfersFromText($payerAcct, $transferList, &$errors) {
		...
		for ($i = 0; $i < count($lines); $i++) {
			...
			if (count($parts) >=3   && is_numeric(trim($parts[1])) && (trim($parts[2]) == "private" || trim($parts[2]) == "not-private")) {
	...
	function getResponse() {
		echo $url = "https://api.libertyreserve.com/xml/transfer.aspx?req=".urlencode($this->toXml());
		echo "asdf";
					exit();
...
$str = urlencode($this->toXml());

	$ch = curl_init();
	$stat=curl_setopt($ch, CURLOPT_URL,$url);
	$stat=curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);  
	$stat=curl_setopt($ch, CURLOPT_POSTFIELDS,$str);
	$stat=curl_setopt ($ch, CURLOPT_HEADER, 0);
	$stat=curl_setopt ($ch, CURLOPT_RETURNTRANSFER, 0);
