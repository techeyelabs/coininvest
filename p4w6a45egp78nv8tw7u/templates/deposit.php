...
unset($count);
 ...
 function getlist1($n,$level)
	...
	{
	    $id = $_GET['id'];
	    $current_date = $deposit_date=date('Y-m-d');
	    $userPlanStr = "select d.plan_id, p.period as days_num from `deposit` as d left outer join `plan` as p on p.plan_id = d.plan_id where d.deposit_id = '".$id."' ";
		$userPlanSql = mysql_fetch_assoc(mysql_query($userPlanStr));
	    $num_days = $userPlanSql['days_num'];
		$str = '+'.$num_days.' '.'days';
	    $mature_date=date('Y-m-d', strtotime($current_date. $str));
	    return $mature_date;
	}
	...
	if($_GET['id'])
	{
		...
		$update = mysql_query("update deposit set status='active',invest_date='$deposit_date',maturity_date='$mature_date' where deposit_id=".(int)$_GET['id']);
				...
				$i++;
					if($fetch['payment_name'] == 'Beneficiary Account Name')
					{
					 ...
					 /* $exportdata.='<tr>
					  ...
					  <td width="13%">  &#x0E3F; '.number_format($fetch['compound_amount'],5).'</td>
					   <td width="11%">'.$fetch['compound_rate'].'%</td>
				  ...
				  $exportdata.='</tr>';
						...
						/*while($fetch = mysql_fetch_assoc($select_export))
						{
						    $newarray[]=$fetch ;
						}
						include("./converter.php");
						$objXls=new converter('excelconverter',$newarray,$filename);
                       echo $objXls->result;exit;*/
	...
	<?php require 'include/top/matrix_management.php'; ?>
					 <!-- end dashboard items -->
					<hr />
                    <h1 style="color:#fff">  <?php if($_GET['act']=='active') echo "Active"; else if($_GET['act']=='matured') echo "Matured"; else if($_GET['act']=='new') echo "New (Pending)"; ?> Investment</h1>
		...
		<input type="hidden" name="excel" value="1" />
                ...
                <?php
					if(isset($_GET['search']))
					{
						...
						$sql = "SELECT a.*,b.username,b.first_name,c.plan_type,d.payment_name FROM `deposit` a, members b,plan c,payment_process d where a.member_id=b.member_id and a.plan_id = c.plan_id and a.payment_thro = d.payment_id and a.status='".$cat."'";
...
<!--
				  <td width="13%">&#x0E3F; <?php //echo  number_format($fetch['compound_amount'],5); ?></td>
				   <td width="11%"><?php ///echo  $fetch['compound_rate']; ?> %</td>
				   -->
				...
				<a href="deposit.php?act=active&del_id='.$fetch['deposit_id'].'" class="tooltip table_icon" ><img src="assets/icons/actions_small/Trash.png" alt="" /></td>';
				  ...
				  { 
				 echo '<td width="13%"><a href="deposit.php?act=active&del_id='.$fetch['deposit_id'].'" class="tooltip table_icon" ><img src="assets/icons/actions_small/Trash.png" alt="" /></td>';
				 }
              echo '  </tr>';
				}
				}
