	...
	if($_POST['submit'])
	{
		...
		*/
		...
		if($flad != 1)
		{
			...
			$update = mysql_query("update plan set plan_name='$plan_name', register_amt='$register_amt', admin_amount='$admin_amount', no_downline='$no_downline' where plan_id=".$_GET['id']);
			
			$_SESSION['success_flag']='<font color="#006600">Your Plan has Successfully Updated</font>';
		...
		else
		{
			...
			echo '<meta http-equiv="refresh" content="0;url=edit_plan.php?id='.$_GET['id'].'">';
			exit();
		}
...
?>
<form name='form1' method='post' action="edit_plan.php?id=<?=$_GET['id']; ?>" >
...
<h1>Settings</h1>
				 ...
				 <? if($_SESSION['succ_dir']) 
					...
					?>
					<fieldset>
						<legend>Edit Your Plan</legend>
						
						
						<p>
							<label>Plan Name:</label>
							<input class="mf" name="plan_name" type="text" value="<?=$fetch['plan_name']; ?>" /> <? if($_SESSION['empty_plan_name']) { echo $_SESSION['empty_plan_name']; $_SESSION['empty_plan_name']=''; } ?>
							
						</p>
						<p>
							<label>Registration Amount:</label>
							<input class="mf" name="register_amt" type="text" value="<?=$fetch['register_amt'];?>" /> USD <? if($_SESSION['empty_register_amt']) { echo $_SESSION['empty_register_amt']; $_SESSION['empty_register_amt']=''; } ?>
							
						</p>
						
						<p>
							<label>Admin Amount :</label>
							<input class="mf" name="admin_amount" type="text" value="<?=$fetch['admin_amount']; ?>" /> USD <? if($_SESSION['empty_admin_amount']) { echo $_SESSION['empty_admin_amount']; $_SESSION['empty_admin_amount']=''; } ?>
							
						</p>
						
						<p>
							<label>Referral Limits :</label>
							<input class="mf" name="no_downline" type="text" value="<?=$fetch['no_downline']; ?>" /> Nos <? if($_SESSION['empty_no_downline']) { echo $_SESSION['empty_no_downline']; $_SESSION['empty_no_downline']=''; } ?>
						...
						<p><input class="button admin-btn" type="submit" value="Submit" name="submit" /></p>
					</fieldset>
					<div class="clearboth"></div>
