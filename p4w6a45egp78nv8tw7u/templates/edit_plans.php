...
<option value="5" <? if($fetch['period_type']=='5') { ?> selected="selected" <? } ?>>Hourly</option>
<option value="1" <? if($fetch['period_type']=='1') { ?> selected="selected" <? } ?>>Day</option>
<option value="2" <? if($fetch['period_type']=='2') { ?> selected="selected" <? } ?>>Week</option>
<option value="3" <? if($fetch['period_type']=='3') { ?> selected="selected" <? } ?>>Month</option>
<option value="4" <? if($fetch['period_type']=='4') { ?> selected="selected" <? } ?>>Year</option>
</select>

<? if($_SESSION['empty_period_type']) { echo $_SESSION['empty_period_type']; $_SESSION['empty_period_type']=''; } ?> 
...
<!--
		<table width="50%" border="1">
				...
				if($fetch['interest_type'] == '2')
				{
					echo ' <input type="checkbox" checked="checked" class="_iphone" name="_interest" id="_interest" value="1"/>';
				}
				else
				{
					echo ' <input type="checkbox"  class="_iphone" name="_interest" id="_interest" value="1"/>';
				}
				?> 
                      </td>
                </tr>
...
<!--
		<table width="50%" border="1">
				...
				{
					echo ' <input type="checkbox" checked="checked" class="_iphone" name="_bouns" id="_bonus" value="yes"/>';
				}
				else
				{
					echo ' <input type="checkbox"  class="_iphone" name="_bouns" id="_bonus" value="yes"/>';
				}
				?>
               </td>
                     <?php 
...
-->
                ...
                <td align="left" width="37%"><label>Release Deposit  <font color="#FF0000">*</font></label></td>

             

                <td align="left" width="63%" onclick="setbouns()" style="padding-left:5px;" >

                 <?php 

				 

				/*if($fetch['release_status'] == 'on')

				{

					echo ' <input type="checkbox" checked="checked" class="iphone" name="release_status" id="release_status" value="yes"/>';
...

				{
					...
					echo ' <input type="checkbox"  class="iphone" name="release_status" id="release_status" value="yes"/>';
				...
				?>       
                       ...
                       <input name="period" type="text" class="mf" id="period" value="<?=$fetch['period']; ?>"  onkeyup="numchk(this)" onkeydown="numvalchk(this)"  />&nbsp;Days<input name="period_type111" type="hidden" class="mf" id="period_type" value="1"/><!--<select name="period_type"><option value="1">Days</option><option value="2">Weeks</option><option value="3">Months</option><option value="4">Years</option></select>--><? if($_SESSION['empty_period']) { echo $_SESSION['empty_period']; $_SESSION['empty_period']=''; } ?>

                             

						</p>

                        <input type="hidden" value="1" name="iperiod_type" /> 
                        <!--
                         <p>
...
<option value="">Select</option>
<option value="5" <?php //if($fetch['iperiod_type']=='5') { ?> selected="selected" <?php //} ?>>Hourly</option>
<option value="1" <?php  //if($fetch['iperiod_type']=='1') { ?> selected="selected" <?php //} ?>>Daily</option>
<option value="2" <?php //if($fetch['iperiod_type']=='2') { ?> selected="selected" <?php //} ?>>Weekly</option>
<option value="3" <?php //if($fetch['iperiod_type']=='3') { ?> selected="selected" <?php //} ?>>Monthly</option>
<option value="4" <?php //if($fetch['iperiod_type']=='4') { ?> selected="selected" <?php //} ?>>Yearly</option>
</select>
...
<?php // if($_SESSION['empty_iperiod_type']) { echo $_SESSION['empty_iperiod_type']; $_SESSION['empty_iperiod_type']=''; } ?> 
              ...
              /* if($fetch['withdrawal_type'] == 'Manual')
				...
				?>
						...
						<!--</p>-->
						 ...
						 $select_plan_level = mysql_query("select * from level_commission where plan_id=".$_GET['id']." ORDER BY `level_commission`.`level_name` ASC");
						 $count_level = mysql_num_rows($select_plan_level);
...

						 {
							 ...
							 $newout.='<input type="hidden" value="'.$count_level.'" id="noofbid" name="noofbid">
							 ...
							 while($fetch_level_commission = mysql_fetch_array($select_plan_level))
                ...
                <td align="left" width="16%"><label>Level '.$fetch_level_commission['level_name'].' <font color="#FF0000">*</font></label></td>
         ...
         <input type="text" id="bonuslevel" class="mf"  value="'.$fetch_level_commission['level_commission'].'" style="margin:5px;" onkeyup="numchk(this)" name="bonuslevel[]">&nbsp;(%)
        ...
        <a href="javascript:;" onclick="addEvent();" class="txt9">Add</a>
                ...
                <td align="left" width="16%"><label>Level '.$fetch_level_commission['level_name'].' </label></td>
				...
				$i++;
				 ...
				 ?>
                  ...
                  <input type="hidden" name="id" value="<?=$fetch['plan_id']; ?>" />
                            ...
                            <input class="button admin-btn" type="submit" id="update" name="update" value="Update" />
