	   ...
	   $update32=mysql_query("update admin_settings set set_value='$mailnotification' where set_id = 75");
		...
		if($transpass!='')
		{
		  ...
		  "N","P","Q","R","S","T","U","V","W","X","Y","Z");
					...
					//echo "select * from history where user_id='$mem_id1'"; exit;
					 ...
					 $amt       = $query1['amount'];
						...
						if($fetch101['set_value'] == 'on')
						{
							...
							$mail_fetch=mysql_fetch_array(mysql_query("select * from mail_subjects where mail_id =13"));
	...
	if($sendpenalty==2 && $userflag != 1)
	{
		...
		while($fetch=mysql_fetch_array($sql))
		{
		 ...
		 if($user_id!='0')
		 {
			...
			$allins==mysql_query("insert into history(user_id,amount,type,description,transaction_id) values('$userid','$amount','penality','$desc','$transaction_id')");
			           if($fetch101['set_value'] == 'on')
						{
					    $current_url="http://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'];
						$current_url=str_replace("updatetranspwd","tran_verification",$current_url);
			...
			$_SESSION['success_flag']='<font color="#006600">Penalty Created Successfully</font>';

			echo '<meta http-equiv="refresh" content="0; url=penalty.php" />';
			exit;
	}
	
	if($sendpenalty==3 && $userflag != 1)
	{
		...
		if(mysql_num_rows($sql1)>0)
		{
			while($fetch=mysql_fetch_array($sql1))
			{
					...
					$transaction_id = "BON".rand(0,9999999);
					$depins=mysql_query("insert into history(user_id,amount,type,description,transaction_id) values('$userid','$amount','penality','$desc','$transaction_id')");
		...
		else if($sendpenalty==4 && $userflag != 1)
		{
      ...
      //echo mysql_num_rows($sql2); exit;
		...
		$transaction_id = "BON".rand(0,9999999);
			...
			//echo "select * from members where member_id = '".$fetch['member_id']."'"; exit;
				...
				//echo "insert into history(user_id,amount,type,description,transaction_id) values('$userid','$amount','bonus','$desc','$transaction_id')";exit;
			 	...
			 	$depnins=mysql_query("insert into history(user_id,amount,type,description,transaction_id) values('$userid','$amount','penality','$desc','$transaction_id')");
		...
		$_SESSION['error_flag']='<font color="#006600">Penalty Created Successfully</font>';
		...
		{
			$transaction_id = "PEN".rand(0,9999999);
			$update=mysql_query("insert into history(user_id,amount,type,description,transaction_id) values('$userid','$amount','penality','$desc','$transaction_id')");
			$_SESSION['success_flag']='<font color="#006600">Penality Created Successfully</font>';
			echo '<meta http-equiv="refresh" content="0; url=penalty.php" />';
			exit();
		}
...
<div id="primary_right">
  ...
  <?php if($_SESSION['empty_pass'] != '')
      ...
      <form name='form1' method='post' action="penalty.php" >
        ...
        <fieldset>
						<legend><font color="#FF0000">*</font> = Required Fields</legend>
                         ...
                         <p>
							<label>Send Penalty To User<font color="#FF0000">*</font></label>
							: 
                          
                       <select name="sendpenalty" id="selbx" onChange="getcode()">
